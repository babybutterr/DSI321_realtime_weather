...
def load_data(lakefs_path):
    ...
    df = df[df["timestamp"] >= pd.Timestamp("2025-05-14")]
...
# ✅ ตรวจว่า all_times มีข้อมูลหรือไม่
if len(all_times) == 0:
    st.error("❌ ไม่พบข้อมูล PM2.5 ที่สามารถแสดงผลได้ กรุณาตรวจสอบวันเวลา หรือแหล่งข้อมูล")
    st.stop()

# ✅ ตั้งค่า default เฉพาะตอนแรก
